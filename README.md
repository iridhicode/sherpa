...
[![CI](https://github.com/iridhicode/sherpa/actions/workflows/ci.yml/badge.svg)](https://github.com/iridhicode/sherpa/actions)
...
instructions that turn a general-purpose LLM into a focused specialist, like a
PostgreSQL triage engineer, a Kubernetes SRE, or a Dockerfile security reviewer.
...
I built the first version of this at work to help my team troubleshoot production
PostgreSQL issues with AI assistance. This is the generalized, open-source rewrite:
a tiny framework for *any* operational skill.

## Why

Everyone on your team prompts the LLM differently, so answer quality is a lottery.
The debugging checklist that lives in a senior engineer's head (what to look at first,
what the usual suspects are, what output format is useful) is exactly what belongs in
a versioned, reviewable artifact. Skills make prompts what they should have been all
along: **code**.
...
3. `~/.agent-skills/skills/` for your personal library
...
- **`Provider` is a `Protocol`**, not a base class. The whole test suite runs
  ...
  new class with nothing else to change.
...
```console
git clone https://github.com/iridhicode/sherpa
cd sherpa
pip install -e ".[dev]"
